...
function toggleSection(sectionId) {
  const div = document.querySelector(`.sidebar-section[data-section-id="${sectionId}"]`);
  const wasOpen = div.classList.contains('open');
  div.classList.toggle('open');

  // Update header active state
  const header = div.querySelector('.sidebar-section-header');
  header.classList.toggle('active', div.classList.contains('open'));
}
...
function showHome() {
  ...
  content.innerHTML = `
    ...
    <div class="home-sections">
      ${SECTIONS.map(s => `
        <a class="section-card" href="#/section/${s.id}" onclick="event.preventDefault(); navigate('#/section/${s.id}')">
  ...
  content.innerHTML = `
    ...
    <div class="guide-content">
      ...
      <div style="display:flex;flex-direction:column;gap:4px;margin-top:8px;">
        ${section.guides.map(g => `
          <a class="sidebar-guide-link" style="padding:8px 12px;border-radius:7px;border:1px solid var(--border);margin:0;display:flex;align-items:center;justify-content:space-between;"
             href="#/guide/${g.slug}"
             onclick="event.preventDefault(); navigate('#/guide/${g.slug}')">
...
async function showGuide(slug) {
  ...
  try {
    ...
    // Parse: first line is metadata, rest is HTML
    const lines = raw.split('\n');
    const meta = lines[0]; // DATA:sectionid:title:parent
    ...
    content.innerHTML = `
      <div style="padding:40px 0">
        <h1 style="color:var(--text);margin-bottom:12px">Guide not found</h1>
        <p style="color:var(--text-muted)">This guide could not be loaded.</p>
        <a href="#/" onclick="event.preventDefault(); navigate('#/')" style="color:var(--accent)">← Back to home</a>
      </div>
    `;
  }
}
  ...
  toc.innerHTML = `
    ...
    <ul class="toc-list">
      ${Array.from(headings).map(h => `
        <li>
          <a href="#${h.id}" class="${h.tagName === 'H3' ? 'toc-h3' : ''}"
             onclick="event.preventDefault(); scrollToHeading('${h.id}')">
            ${h.textContent}
          </a>
        </li>
      `).join('')}
...
function scrollToHeading(id) {
  ...
  if (el) {
    const offset = parseInt(getComputedStyle(document.documentElement).getPropertyValue('--header-height')) + 16;
...
function setupTOCScroll(headings) {
  ...
  }, { rootMargin: `-${56+16}px 0px -70% 0px` });
...
function buildGuideNav(slug, section) {
  ...
  placeholder.innerHTML = `
    <div class="guide-nav">
      ${prev ? `
        <a class="guide-nav-link prev" href="#/guide/${prev.slug}"
           onclick="event.preventDefault(); navigate('#/guide/${prev.slug}')">
          <div class="guide-nav-label">← Previous</div>
          <div class="guide-nav-title">${prev.title}</div>
        </a>
      ` : '<div></div>'}
      ${next ? `
        <a class="guide-nav-link next" href="#/guide/${next.slug}"
           onclick="event.preventDefault(); navigate('#/guide/${next.slug}')">
          <div class="guide-nav-label">Next →</div>
          <div class="guide-nav-title">${next.title}</div>
        </a>
      ` : '<div></div>'}
    </div>
  `;
    ...
    if (matches.length === 0) {
      ...
      results.innerHTML = matches.map(g => `
        <a class="search-result-item" href="#/guide/${g.slug}"
          ...
          <div class="search-result-section">${g.parent}</div>
        </a>
      `).join('');
    }
